...
export function githubRoutes(): Hono<AppEnv> {
  ...
  app.get("/token", async (c) => {
    ...
    const setting = db.select().from(settings).where(eq(settings.key, GITHUB_TOKEN_KEY)).get();
  ...
  app.post("/token", async (c) => {
    ...
    const existing = db.select().from(settings).where(eq(settings.key, GITHUB_TOKEN_KEY)).get();
