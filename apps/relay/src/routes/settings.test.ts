  ...
  describe("GET /api/settings", () => {
    ...
    it("returns settings as object", async () => {
      db.insert(settings)
        .values({
          key: "default_model",
          value: JSON.stringify({ provider: "anthropic", modelId: "claude-3-opus" }),
      ...
      expect(res.status).toBe(200);
      const json = await res.json();
      expect(json.data.default_model).toEqual({ provider: "anthropic", modelId: "claude-3-opus" });
    });
  ...
  describe("PUT /api/settings", () => {
    ...
    it("stores complex values", async () => {
      ...
      const stored = db
        ...
        .get();
      expect(JSON.parse(stored!.value)).toEqual({ nested: { array: [1, 2, 3] } });
    });
