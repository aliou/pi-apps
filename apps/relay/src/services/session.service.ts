...
export type SessionStatus = "creating" | "ready" | "running" | "stopped" | "error" | "deleted";
...
export class SessionService {
  ...
  update(sessionId: string, fields: UpdateSessionParams): void {
    ...
    this.db.update(sessions).set(updates).where(eq(sessions.id, sessionId)).run();
  }
  ...
  touch(sessionId: string): void {
    const now = new Date().toISOString();
    this.db.update(sessions).set({ lastActivityAt: now }).where(eq(sessions.id, sessionId)).run();
  }
}
