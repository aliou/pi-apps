...
export default function ExtensionsPage() {
  ...
  const [catalogMeta, setCatalogMeta] = useState<{ fetchedAt: string | null; stale: boolean }>({
    fetchedAt: null,
    stale: false,
  });
  ...
  const loadInstalled = useCallback(async (activeScope: ExtensionScope) => {
    setLoading(true);
    const res = await api.get<ExtensionConfigRecord[]>(`/extension-configs?scope=${activeScope}`);
    if (res.error) {
      setError(res.error);
      setConfigs([]);
    } else {
  ...
  const installedNames = useMemo(() => new Set(configs.map((item) => item.package)), [configs]);
  ...
  const handleSaveConfig = async () => {
    ...
    setSaving(true);
    setFieldErrors({});
    const res = await api.put<ExtensionConfigRecord>(`/extension-configs/${selected.id}`, {
      config: draftConfig,
    });
    setSaving(false);
    if (res.error) {
      setError(res.error);
      const meta = (res as { meta?: { fieldErrors?: Array<{ field: string; message: string }> } }).meta;
      const nextErrors = Object.fromEntries((meta?.fieldErrors ?? []).map((entry) => [entry.field, entry.message]));
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-muted">
          Discover Pi packages, inspect manifest metadata, and store extension config by scope.
        </p>
      </div>
      ...
      <Tabs value={view} onValueChange={(details) => setView(details.value as ViewTab)}>
        <Tabs.List className="gap-1 rounded-lg border border-border bg-bg p-1">
          <Tabs.Trigger value="installed" className="rounded-md border-none px-3 py-1.5 text-xs data-[selected]:text-fg">
            Installed
          </Tabs.Trigger>
          <Tabs.Trigger value="discover" className="rounded-md border-none px-3 py-1.5 text-xs data-[selected]:text-fg">
            Discover
          </Tabs.Trigger>
          <Tabs.Trigger value="skills" className="rounded-md border-none px-3 py-1.5 text-xs data-[selected]:text-fg">
            Skills
          </Tabs.Trigger>
        ...
        <div className="space-y-4">
          ...
          {catalogMeta.fetchedAt ? (
            <p className="text-xs text-muted">
              {catalogMeta.stale ? "Showing cached results." : "Catalog refreshed."} {catalogMeta.fetchedAt}
            </p>
          ) : null}
          {loading ? (
            ...
            <div className="grid gap-3 md:grid-cols-2">
              {catalog.map((pkg) => (
                <div key={pkg.name} className="rounded-xl border border-border bg-surface/20 p-4">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <h3 className="text-sm font-semibold text-fg">{pkg.name}</h3>
                      <p className="mt-1 text-xs text-muted">{pkg.description ?? "No description."}</p>
                    </div>
                    <Button
                      variant={installedNames.has(pkg.name) ? "secondary" : "primary"}
                      size="sm"
                      loading={saving}
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    {(pkg.extensionMeta?.tools ?? []).map((value) => (
                      <span key={value} className="rounded-md border border-border px-2 py-1 text-xs text-fg">
                        tool:{value}
                      </span>
                    ))}
      ...
      {view === "installed" ? (
        loading ? (
          ...
          <div className="space-y-3">
            {configs.map((item) => (
              <div key={item.id} className="flex items-center gap-3 rounded-xl border border-border bg-surface/20 p-4">
                <SparkleIcon className="size-5 text-accent" />
                <div className="min-w-0 flex-1">
                  <div className="truncate text-sm font-semibold text-fg">{item.package}</div>
                  <div className="truncate text-xs text-muted">
                    {item.manifest?.description ?? "Manifest unavailable. Install still allowed."}
                  </div>
                </div>
      ...
      {view === "skills" ? (
        loading ? (
          ...
          <div className="space-y-3">
            {skills.map((item) => (
              <div key={`${item.package}:${item.skill}`} className="rounded-xl border border-border bg-surface/20 p-4">
                <div className="flex items-start gap-3">
                  <WrenchIcon className="mt-0.5 size-5 text-accent" />
                  <div>
                    <div className="text-sm font-semibold text-fg">{item.skill}</div>
                    <div className="text-xs text-muted">Exposed by {item.package}</div>
