...
public struct DiffView: View {
    ...
    public var body: some View {
        if SharedMetalResources.shared.device != nil {
            #if os(macOS)
            DiffViewRepresentable_macOS(patches: patches)
                .frame(minHeight: 200)
            #else
            DiffViewRepresentable_iOS(patches: patches)
                .frame(minHeight: 200)
            #endif
        } else {
...
#if os(macOS)
struct DiffViewRepresentable_macOS: NSViewRepresentable {
    let patches: [DiffPatchInput]
...
#if os(iOS)
struct DiffViewRepresentable_iOS: UIViewRepresentable {
    let patches: [DiffPatchInput]
