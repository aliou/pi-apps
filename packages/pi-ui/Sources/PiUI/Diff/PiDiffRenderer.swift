...
public class PiDiffRenderer: NSObject, MTKViewDelegate {
    ...
    private func buildPipelines() {
        ...
        guard let library = device.makeDefaultLibrary()
            ?? (try? device.makeLibrary(source: PiDiffRenderer.shaderSource, options: nil))
        else { return }
    ...
    func updateInstances(_ instances: [InstanceData], boldInstances: [InstanceData] = [], rects: [RectInstance]) {
        ...
        if instanceCount > 0 {
            ...
            if shouldRecreateInstanceBuffer {
                ...
                newBuffer?.label = "PiDiffRenderer.InstanceBuffer"
        ...
        if boldInstanceCount > 0 {
            ...
            if shouldRecreateBoldBuffer {
                let optimalSize = Self.optimalBufferSize(for: size)
                let newBuffer = device.makeBuffer(length: optimalSize, options: .storageModeShared)
                newBuffer?.label = "PiDiffRenderer.BoldInstanceBuffer"
        ...
        if rectCount > 0 {
            ...
            if shouldRecreateRectBuffer {
                ...
                // CRITICAL: Create new buffer BEFORE releasing old one to avoid GPU stalls
                let newBuffer = device.makeBuffer(length: optimalSize, options: .storageModeShared)
                newBuffer?.label = "PiDiffRenderer.RectBuffer"
